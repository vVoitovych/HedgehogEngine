    ...
    inline bool RenderPropertyWidget(void* comp, const PropertyDescriptor& prop)
    {
        ...
        case TypeTag::Double:
            { float tmp = static_cast<float>(*FieldPtr<double>(comp, prop));
              if (ImGui::DragFloat(prop.name, &tmp)) { *FieldPtr<double>(comp, prop) = static_cast<double>(tmp); changed = true; } }
            break;
        case TypeTag::UInt:
            { int tmp = static_cast<int>(*FieldPtr<uint32_t>(comp, prop));
              if (ImGui::DragInt(prop.name, &tmp, 1.0f, 0, INT_MAX)) { *FieldPtr<uint32_t>(comp, prop) = static_cast<uint32_t>(tmp); changed = true; } }
            break;
        case TypeTag::Int:
            if (HasFlag(prop.flags, PropertyFlags::IsSlider))
        ...
        case TypeTag::String:
            { auto* str = FieldPtr<std::string>(comp, prop);
              ...
              if (ImGui::InputText(prop.name, buf, sizeof(buf))) { *str = buf; changed = true; } }
            break;
        case TypeTag::Vec2:
