...
namespace Wrappers
{
        ...
        , m_GraphicsPipelineLayout(nullptr)
    {
        ...
        if (vkCreatePipelineLayout(device.GetNativeDevice(), &layoutCreateInfo, nullptr, &m_GraphicsPipelineLayout) != VK_SUCCESS)
        {
    ...
    Pipeline::~Pipeline()
    {
        ...
        {
            LOGERROR("Vulkan pipeline should be cleaned up before destruction!");
            ENGINE_DEBUG_BREAK();
        }
        if (m_GraphicsPipelineLayout != nullptr)
        {
            LOGERROR("Vulkan pipeline layout should be cleaned up before destruction!");
            ENGINE_DEBUG_BREAK();
        }
    }
    ...
    Pipeline::Pipeline(Pipeline&& rhs) noexcept
        ...
        , m_GraphicsPipelineLayout(rhs.m_GraphicsPipelineLayout)
    {
        rhs.m_Pipeline = nullptr;
        rhs.m_GraphicsPipelineLayout = nullptr;
    }
    ...
    void Pipeline::Cleanup(const Device& device)
    {
        ...
        vkDestroyPipelineLayout(device.GetNativeDevice(), m_GraphicsPipelineLayout, nullptr);
