...
namespace Renderer
{
	...
	void Renderer::Cleanup()
	{
		...
		mMeshContainer.Cleanup(mDevice);
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
	...
	void Renderer::DrawFrame()
	{
		...
		VkBuffer vertexBuffers[] = { mMeshContainer.GetVertexBuffer() };
		VkDeviceSize offsets[] = { 0 };
		commandBuffer.BindVertexBuffers(0, 1, vertexBuffers, offsets);
		commandBuffer.BindIndexBuffer(mMeshContainer.GetIndexBuffer(), 0, VK_INDEX_TYPE_UINT32);
		...
		auto& mesh = mMeshContainer.GetMesh(0);
